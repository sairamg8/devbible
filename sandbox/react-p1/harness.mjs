...
/**
 * Scratch space lives **inside this sandbox**, never the host's /tmp — derived
 * from this file's own location so it is correct no matter where the script is
 * run from. Evidence behind a committed script has to be reproducible, and a
 * path in /tmp is invisible to git, shared with every other session on the
 * machine, and gone after a reboot.
 */
...
export async function bundle(source, {mode = 'development', jsx = true} = {}) {
  ...
  const dir = await mkdtemp(join(SCRATCH, 'bundle-'));
