...
description: "Keep devbible current with upstream release cycles. Use when the weekly currency workflow opens an issue, when a product ships a release, or on \"is X still current\", \"update the bible for <version>\", \"bump <product>\". Triages version drift by class, bumps only what the drift class licenses, and never re-reads a page for a patch."
---
