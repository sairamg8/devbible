---
name: devbible-prompt-javascript-chunk-d
description: Paste-ready bootstrap prompt for JavaScript chunk D (phases 12 and 18 — the browser platform, then the three kept storefront topics) — hand it to a fresh session and it starts cold
metadata:
  type: reference
---

**Paste everything below the line into a fresh session.** Or just type **`pick javascript D`** —
`~/.claude/CLAUDE.md` §11b now recognises that on its own and starts the same work without asking.

---

You are taking **JavaScript chunk D** of the devbible corpus: **phases 12 and 18 — the browser platform, then the three kept storefront topics**. **22 topics left.**
...
2. `/run/media/sairam/Storage/my-learning/claude/devbible/progress_javascript_split_4way.md` — the
   live cursor for all four chunks. **Yours is chunk D.**
...
|---|---|---|---|---|
| **12 · The browser platform** | 2/21 | **19** | **03 · Timers and frames (Understand)** | `docs/javascript/pages/phase-12-browser-platform/` |
| **18 · Building the store front end** | 7/18 | **3** | **11 · Infinite scroll and lazy images** | `docs/javascript/pages/phase-18-storefront/` |

The three storefront topics that survived the scope cut — infinite scroll, long lists, review uploads — are exactly the observer / worker / File API work phase 12 covers, so writing them together avoids saying the same thing twice.

**Phase 12 is the thinnest phase in the corpus (2 of 21)**, which is why this chunk pairs it with only three other topics.
...
**Phase 12 · The browser platform — 19 left.** Start at **03 · Timers and frames (Understand)**, then 04 `IntersectionObserver` · 05 `ResizeObserver` · 06 `PerformanceObserver` · 07 Web Workers · 08 The History API and client-side routing · 09 `window`, `document`, `navigator`, `screen` · 10 WebCrypto · 11 Accessibility from JavaScript · 12 Feature detection · 13 What belongs on the server — then **Know**: 14 Yielding to the main thread · 15 Cross-tab coordination · 16 Clipboard, Web Share, File System Access · 17 Permissions, Geolocation, Notifications · 18 Media · 19 Page Visibility, Wake Lock, Battery · 20 Internationalisation in the browser — then **When Needed**: 21 `SharedArrayBuffer` and `Atomics`.

**Phase 18 · Building the store front end — 3 left.** Start at **11 · Infinite scroll and lazy images**, then 12 Long lists without freezing · 15 Review uploads. ⚠️ **ONLY these three.** Topics 08–10, 13, 14 and 16–18 were **dropped** on 2026-08-14 and are not to be written — a phase that shows unwritten dropped rows is CORRECT, do not tidy it up.
...
- `docs/javascript/pages/README.md` — the `Held by` cell of the chunk D row.
- `docs/README.md` — the `Claimed by` cell of the **JavaScript · chunk D** row.
...
**phase 12 topic 03 · Timers and frames (Understand)** — no plan, no confirmation.
