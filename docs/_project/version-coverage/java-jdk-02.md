...
description: java-jdk version coverage, part 2 of 3 — §3.4–§3.8 the delta table for JDK 22 → 27 and the previews (rows 79–161), and §4 the summary by LTS line. Continues java-jdk.md; §5 hand-off is in java-jdk-03.md.
metadata:
  type: project
---
...
| 151 | 24→27 | Legacy launcher options: `-Xfuture`, `-t`, `-tm`, `-checksource`, `-cs`, `-noasyncgc` removed (24); `-verbosegc`, `-ms`, `-mx`, `-ss`, `-verify` deprecated (24); `-noclassgc`, `-noverify`, `-verifyremote`, `-Xverify:none` removed (27) | removed | PLANNED | syllabus topic "JVM flags that matter in 2026 — and the ones that stopped mattering", `syl/04-production.md:61`; its retired-flags chunk `06-the-retired-list.md` is planned (`p12/13-jvm-flags-that-matter/_plan.md:24`) and unwritten — topic 13 is marked *(not written yet)* in `p12/README.md:41`. Terms `Xverify:none`, `noverify`, `-noclassgc`, `-verbosegc` → 0 hits today | RN24 JDK-8339918, JDK-8286851, RN27 JDK-8373481 |
