...
> Verified: 2026-09-03 against Python 3.14 Language Reference, What's New in Python 3.14, PEP 649, PEP 749, annotationlib module.
...
| # | Chunk | Covers |
|---|---|---|
| 1 | **[Deferred evaluation and __annotate__](./01-deferred-evaluation-and-the-annotate-protocol.md)** | Eager evaluation vs stringification vs deferred evaluation; compiler-generated `__annotate__` code objects; Format 1 (VALUE), 2 (FORWARDREF), 3 (SOURCE); legacy `__annotations__` backward compatibility |
...
You are done with this topic when you can explain how Python 3.14 evaluates annotations lazily, inspect annotations using `annotationlib.get_annotations()`, choose between `Format.VALUE` and `Format.FORWARDREF`, and explain why `from __future__ import annotations` is no longer needed.
