...
- **01 · `list` internals** *(not written yet)* is the dynamic array whose iteration-while-mutating failure is silent where the dict's is loud.
- **02 · `tuple`** *(not written yet)* is the hashable-if-its-contents-are key type.
