...
- Neither overwrites a file it did not create. uv: *"Installation of tools will not overwrite executables in the executable directory that were not previously installed by uv. For example, if `pipx` has been used to install a tool, `uv tool install` will fail."* pipx logs *"File exists at … Not modifying."* Collisions are **10** *(not written yet)*.
