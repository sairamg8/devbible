...
| 7 | **[06b · Plugin hosts in the wild](06b-plugin-hosts-in-the-wild.md)** | pytest autoloads every `pytest11` module (the documented start-up order) and ships `-p no:`, `PYTEST_DISABLE_PLUGIN_AUTOLOAD` and `--disable-plugin-autoload` (8.4) — 🔴 installed packages are test configuration; flake8's entry-point name is the error-code prefix, its own `F`/`E`/`W` are entry points, duplicates *"deactivated without warning"*, its finder dedupes and skips metadata parsing; pipx's `pipx.run` group fixes a package/command name mismatch |
| 8 | **[07 · `uv run` and the project command](07-uv-run-and-the-project-command.md)** | the chain — sync, editable install (needs a build system), wrapper in `.venv/bin`, scripts dir prepended to the child's `PATH` (`run.rs`); bare `uv run` lists commands, *"Failed to spawn"* means none matched; 🔴 uv re-installs an editable project when `pyproject.toml` changes, so a new script appears on the next run (settles 02c) — except dynamic metadata (`cache-keys`) and `--no-sync`; `uv run file.py` is a script, not the command; not an `ENTRYPOINT` |
| 9 | **[08 · Getting commands to users](08-getting-commands-to-users.md)** | `uv tool install`, `uvx` and `pipx` expose only the named package's scripts — 🔴 a package with no `[project.scripts]` is refused, a dependency's command is invisible without `--with-executables-from` (uv 0.8.5) or `--include-deps`; name ≠ package needs `uvx --from` or a `pipx.run` entry; 🔴 entry-point plugins must go *into* the tool's environment (`--with`, `pipx inject`); `PATH` warnings, `update-shell`, `ensurepath --prepend`; pipx 1.17.2 defaults to a uv backend; editable tool installs keep code live, not commands |
| 10 | **[09 · Stale wrappers and editable installs](09-stale-wrappers-and-editable-installs.md)** | editable keeps code live and wrappers + `entry_points.txt` frozen — the live/frozen table for pip, uv projects and uv tools; 🔴 a renamed function passes build and install and fails when the command runs, so test that every installed entry point `load()`s; reinstall removes old wrappers via recorded metadata unless it was deleted or clobbered; dead shebangs after a move, an uninstalled Python, or the user scheme's shared `bin`; a dead-wrapper audit script |
| 11 | **[10 · Name collisions and `PATH` shadowing](10-name-collisions-and-path-shadowing.md)** | names are unique only within a distribution — 🔴 in one environment the last install wins (pip `clobber = True`) and uninstalling either package deletes the shared file; reserved interpreter names refused by uv; across directories bash checks aliases, functions and builtins before `PATH`, then caches the path (`hash -r`); `type -a` shows every candidate; uv and pipx refuse to overwrite each other in `~/.local/bin`; `ensurepath` appends unless `--prepend`; naming rules and a collision check for CI |
| 12 | **[11 · The import-time cost of a CLI](11-import-time-cost.md)** | the wrapper imports the entry point's module and its whole top-level graph on every run — `--help`, typos, completion (PEP 810's *"several seconds"*); measure with `-X importtime` / `PYTHONPROFILEIMPORTTIME` (3.14 adds `=2`); 🔴 a thin `__init__.py`, subcommand imports inside handlers, type-only imports under `TYPE_CHECKING` (3.14 defers annotations), plugins loaded on demand; PEP 810 lazy imports are 3.15, not 3.14; a `sys.modules` budget test instead of timings |
