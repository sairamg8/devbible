...
disassembly — see topic [12 · Bytecode inspection with `dis`](../12-dis-bytecode.md)
...
**Fix:** stop asking the question in code. If you are genuinely investigating, `dis` will show both names loading the same `LOAD_CONST` index — see topic [12](../12-dis-bytecode.md) — but the answer is not portable and must not be relied on
