...
🚧 **Phase in progress — 0 of 12 written.**

| # | Page | Tier | In one line |
|---|---|---|---|
| 01 | **What Python is: CPython** | <span className="db-tier t-master">Master</span> | Source → bytecode → the interpreter loop; the language vs its implementation |
| 02 | **The GIL** | <span className="db-tier t-master">Master</span> | One thread runs bytecode — but I/O releases it, and 3.14 makes free-threading official |
...
| 07 | **Everything is an object** | <span className="db-tier t-master">Master</span> | Names bind to objects, `is` vs `==`, and why `a = b` never copies |
| 08 | **Imports** | <span className="db-tier t-master">Master</span> | Modules, packages, `sys.path` — and the file you named `random.py` |
...
| 11 | **Startup and import cost** | <span className="db-tier t-know">Know</span> | Why CLIs feel slow, and lazy imports as the coming answer |
