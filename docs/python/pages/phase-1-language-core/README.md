...
🚧 **In flight — 7 of 16.** Complete: **01 · Syntax and indentation** (8 chunks + index, 1,900 lines), **02 · Numbers** (69 + index, 17,166), **03 · Strings** (9 + index, ~2,150), **04 · `bytes` vs `str`** (4 + index, ~1,100), **05 · Truthiness** (12 + index, 3,261), **06 · Comparisons** (23 + index, 5,531) and **08 · Control flow** (6 + index, 1,692). **Topics 05, 06 and 08 closed 2026-09-02** — truthiness from the falsy rule through the empty-versus-missing bug, `and`/`or` returning operands and `all([])` being `True`; comparisons from the rich-comparison protocol through `is` versus `==`, the small-int cache, cross-type ordering, sorting and the two big libraries where `==` is not a boolean; and control flow from the `for` statement's actual mechanism through `zip`'s silent truncation, the `for`/`else` clause that means *no break*, and the list iterator whose marching index makes mid-loop mutation skip elements without raising. All three closed at **0 over the 300-line cap, 0 MDX hazards, 0 dangling links, unbroken footer chains and no duplicate `sidebar_position`.** **Topic 07 · Assignment and aliasing is being written now** and is committed as an in-flight snapshot, not as a closed topic. Topic 04 is still short one planned chunk. Topics 09–16 are unwritten.
...
| 10 | **`match` — structural pattern matching** *(not written yet)* | <span className="db-tier t-understand">Understand</span> | Destructuring by shape, class patterns, guards |
