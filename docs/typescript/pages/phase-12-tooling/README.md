...
| 04 | [Testing types](./04-testing-types/README.md) *(🚧 1 chunk)* | <span className="db-tier t-understand">Understand</span> | 🔴 The runner is `tsc`, so a type test outside the checked program **cannot fail** — and asserting a call is **rejected** is the half nothing else covers |
