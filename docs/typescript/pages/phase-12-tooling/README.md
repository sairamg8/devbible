...
| 03 | [Build pipelines](./03-build-pipelines/README.md) *(🚧 1 chunk)* | <span className="db-tier t-understand">Understand</span> | 🔴 Four jobs, not one — and **only `tsc` emits `.d.ts`**, so if you publish types the compiler is in your build whatever bundler you chose |
