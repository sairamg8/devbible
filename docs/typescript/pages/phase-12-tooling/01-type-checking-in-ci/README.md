...
Chunk **01 is written**. The rest are not, and references to them are deliberately
...
| 02 | **What the gate guarantees** *(not written yet)* | `tsc --noEmit` checks the program you configured — so `include`/`exclude` and `skipLibCheck` decide the size of the guarantee, and a green run can still be misleading |
| 03 | **Where the gate goes** *(not written yet)* | Its position in the pipeline, and the two-type-check interaction with type-aware linting |
