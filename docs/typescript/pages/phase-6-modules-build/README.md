...
| 05 | [`isolatedModules`](./05-isolatedmodules/README.md) *(6 chunks)* | <span className="db-tier t-understand">Understand</span> | The constraint every single-file transpiler needs, and the patterns it bans |
| 06 | [File extensions — `.ts`/`.mts`/`.cts`/`.d.ts`](./06-file-extensions/README.md) *(5 chunks)* | <span className="db-tier t-understand">Understand</span> | `allowImportingTsExtensions`, `rewriteRelativeImportExtensions`, and writing the extension the *runtime* wants |
...
| 14 | [Incremental builds](./14-incremental-builds/README.md) *(3 chunks)* | <span className="db-tier t-know">Know</span> | `.tsbuildinfo`, what invalidates it, and caching it in CI |
| 15 | [`isolatedDeclarations`](./15-isolateddeclarations/README.md) *(3 chunks)* | <span className="db-tier t-know">Know</span> | Declaration emit without full inference, the annotation cost, and the speed it buys |
| 16 | [Typing non-code imports](./16-typing-non-code-imports/README.md) *(3 chunks)* | <span className="db-tier t-know">Know</span> | CSS modules, JSON, images, and `?raw`-style bundler suffixes |
