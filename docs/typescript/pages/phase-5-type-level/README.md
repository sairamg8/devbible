...
| 14 | [`NoInfer<T>`](./14-noinfer.md) | <span className="db-tier t-know">Know</span> | 🔴 **The only tool in the phase that changes how a type is arrived at rather than what it is** — why a two-position type parameter is symmetric by default, and why marking the wrong position fails silently |
| 15 | [Union → intersection and other identities](./15-union-to-intersection.md) | <span className="db-tier t-know">Know</span> | 🔴 **The union-to-intersection flip is contravariance applied deliberately** — decoded rather than presented, with `IsUnion`/`IsNever`/`IsAny`, why union-to-tuple is a warning, and the honest conclusion: recognise them, do not reach for them |
