...
| **7xxx** | 53 | **The `noImplicitAny` family.** *"…implicitly has an '{1}' type"* | 🔴 **Not a type error — a refusal to insert an implicit `any`.** See [chunk 05](./05-the-index-codes.md) |
...
that are all its fault. [Chunk 09](./09-the-suppress-codes-are-gone.md) is an
