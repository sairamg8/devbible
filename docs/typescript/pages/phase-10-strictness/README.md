...
| 11 | [typescript-eslint type-aware rules](./11-typescript-eslint/README.md) *(🚧 5 of 8 chunks)* | <span className="db-tier t-understand">Understand</span> | ⚠️ The syllabus said *"the checks the compiler will not do"* — **half of that is wrong**: `no-unnecessary-condition` overlaps **seven** compiler codes, so the topic states the exact leftover instead. Plus the CI cost, quoted from typescript-eslint |
