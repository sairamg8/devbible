...
**13 topics — ✅ COMPLETE.** `strict: true` is the start, not the finish.
...
| 12 | [Assertion discipline](./12-assertion-discipline/README.md) *(5 chunks)* | <span className="db-tier t-understand">Understand</span> | Treating every `as` as a review comment, and the guard that should have been written |
| 13 | [Designing APIs `unknown`-first](./13-unknown-first-apis.md) | <span className="db-tier t-know">Know</span> | 🔴 A parameter type is a **promise the compiler cannot keep at a boundary** — so take `unknown` and return the type, making it an *output* of validation rather than an asserted input |
