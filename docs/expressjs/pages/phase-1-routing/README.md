...
> full depth as `NN-topic/` directories. **All three are done** — 01 is 3 chunks (up from 117
> lines), 02 is 3 chunks (up from 145), 03 is 3 chunks (up from 135). Still no runs: the new mechanism claims are read
...
| 03 | **[Router composition](03-router-composition/README.md)** *(3 chunks)* | <span className="db-tier t-master">Master</span> | Modular routers and the four URL properties under a mount; `mergeParams` and the isolation `restore` enforces; the mount list as the only visible pipeline |
...
| `express.Router()` composition | 03 (chunks [01](03-router-composition/01-mounting-a-router.md) · [03](03-router-composition/03-composition-at-scale.md)) |
| Nested routers and prefixes | 03 (chunk [02](03-router-composition/02-mergeparams-and-isolation.md)) |
