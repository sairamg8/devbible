...
|---|---|---|---|
| 01 | **`java.time`** *(not written yet)* | <span className="db-tier t-master">Master</span> | `Instant` in the data, zones at the edge; the DST bug prevented |
...
| 03 | **Streams, buffers and charsets** *(not written yet)* | <span className="db-tier t-understand">Understand</span> | Byte vs char, buffering, UTF-8 default since 18 |
| 04 | **`HttpClient`** *(not written yet)* | <span className="db-tier t-understand">Understand</span> | Sync/async HTTP without a library — timeouts on every call |
| 05 | **JSON with Jackson** *(not written yet)* | <span className="db-tier t-master">Master</span> | One shared `ObjectMapper`; records as DTOs; unknown-field policy |
...
| 07 | **`UUID` and randomness** *(not written yet)* | <span className="db-tier t-understand">Understand</span> | `SecureRandom` vs `Random` vs `ThreadLocalRandom` — one is safe for tokens |
| 08 | **Java serialization** *(not written yet)* | <span className="db-tier t-know">Know</span> | A deserialization-attack liability — recognize and avoid |
...
| 10 | **`ProcessBuilder`** *(not written yet)* | <span className="db-tier t-know">Know</span> | Shelling out safely: argument lists, stream draining |
