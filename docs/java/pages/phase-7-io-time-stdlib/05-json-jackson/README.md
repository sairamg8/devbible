...
| 2 | **[The policy decisions](02-policy-decisions.md)** | Unknown fields (`FAIL_ON_UNKNOWN_PROPERTIES` both ways), null vs absent vs default, `JsonInclude`, `Optional` via `jackson-datatype-jdk8`, `java.time` via `JavaTimeModule`, the annotations that carry weight, generics with `TypeReference` |
