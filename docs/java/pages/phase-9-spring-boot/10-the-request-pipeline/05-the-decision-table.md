...
| **Input sanitising** | **Nowhere as a blanket layer.** Validate at binding; encode at output | A filter that rewrites request bytes corrupts legitimate payloads and gives false assurance. Constraints belong on the DTO (**Topic 08 — Validation** *(not written yet)*), parameterised queries defeat SQL injection, and escaping is the renderer's job |
