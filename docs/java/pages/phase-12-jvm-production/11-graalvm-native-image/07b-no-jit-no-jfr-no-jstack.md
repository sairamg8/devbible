...
- **Leak profiling is partial.** *"Leak profiling implemented using the `jdk.OldObjectSample` event is partially available. Specifically, old object tracking is possible, but the path to the GC root information is unavailable."* ⚠️ **Path-to-GC-root is the entire point of `OldObjectSample` for leak hunting**; without it you know *what* is old, not *why* it is retained. A heap dump ([07d · The diagnostic toolbox](07d-the-diagnostic-toolbox.md)) is the answer instead.
