...
- **Oracle GraalVM is what you need if throughput matters** — G1, `--pgo`, `-O3`/GraalNN and `--emit build-report` are all on that side of the line, and [07c](07c-getting-throughput-back.md) is the argument for why that is not a small difference.
