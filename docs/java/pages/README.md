...
| 10 · Data access | 14 | 🚧 **11/14 closed** — topics 01–08 complete (**335 chunks, ~88,000 lines**): JDBC · connection pooling · JDBC transactions · Spring `@Transactional` · SQL-first access · the JPA/Hibernate model · relationships and fetch types · the N+1 problem. **09 · Spring Data JPA (49 chunks), 13 · jOOQ (33) and 14 · Spring Data Mongo/Redis (26) closed 2026-08-27** with their indexes. **12 · Caching and 10 · Lazy-loading pitfalls both closed 2026-08-27 at 35 chunks each** with their indexes, so the phase stands at **13/14**. Topic **11 · Flyway is part-written — 40 chunks on disk, no index yet**; it resumes from the chunk plan in its own directory. Two chunks that committed pages linked to by name but nobody had written — Flyway's `10c` and caching's `07d` — were found by resolving every link target against the filesystem and have been authored |
