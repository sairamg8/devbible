...
🏁 **COMPLETE — 17 of 17 topics written**, at every tier (Master 6/6 · Understand 7/7 ·
Know 4/4). **28 files, 5,563 lines, largest 286, 0 over the 300-line cap** — six topics
needed a chunk directory rather than a shorter page. Every internal link was resolved
against the filesystem: **link-checked, not built**.
...
| 15 | **[`podman compose` and `podman-compose`](15-podman-compose.md)** | <span className="db-tier t-understand">Understand</span> | Three programs share the name — and `podman compose` is a dispatcher that probably runs `docker-compose` |
| 16 | **[`include` and `extends`](16-include-and-extends.md)** | <span className="db-tier t-know">Know</span> | Two ways to split a large setup with **opposite** relative-path rules — and `extends` does not import the resources it references |
| 17 | **[`--scale` and the honest limits](17-scale-and-limits.md)** | <span className="db-tier t-know">Know</span> | N containers is not scaling — no load balancer, no rescheduling, and `container_name` blocks it outright |
