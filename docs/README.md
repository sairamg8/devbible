...
| ------------------------------------------ | ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **React** — `docs/react/`, SPLIT A/B       | **A:** session `bfcb390b` · **B:** session `05921047` | 2026-08-14 | 🔴 **Active on `main`** — the `react-phase-7` worktree was **merged** (`d74e74f`), nothing is stranded. **Phases 0–10 COMPLETE — 217 pages.** **Part A** = Phase 11 topics 08–17 + phase close (**10/17 written**). **Part B** = Phase 14 · Testing React, 14 topics, **writing — 2/14**. Split rules: `devbible/project_react_split_parts_ab.md`; handoff: `devbible/progress_react_phase7.md`.                                                              |
...
| **JavaScript · lane A** — phases **3–8** (the language) | session `edbfba95` | 2026-08-14 | 🔴 **Active — LANE A of a two-way split (2026-08-14): functions, objects, built-ins, iteration, async, modules. 61 topics left. TIER-LOCKED to Understand and Know.** 🔴 **Every Master tier is COMPLETE, phases 0–18 (99 of 99 topics)** and is not to be reopened for depth. **146 of 316 in-scope topics written (46%)**; **136 remain — Understand 92 · Know 41 · When Needed 3**. 🔴 **Scope cut 2026-08-14 — language focus:** the DSA block parked (13, 14, 15), Dynamic programming **dropped**, storefront trimmed to three. **21 dropped, 34 parked, nothing written deleted**; the original syllabus is preserved as `*.md.bak`. Order is phase by phase, and inside a phase Understand → Know → When Needed. ✅ **Phase 3 is COMPLETE at every tier (20/20).** Now in **phase 4** (12/20) — next: **topic 13 · `instanceof` and `Symbol.hasInstance`**. Phases 0–2 are complete at every tier. Thinnest: phase 12 (2/21), 16 (3/16), 13 (3/10), 6 (3/13). See the [JavaScript claim notice](javascript/pages/README.md) |
| **JavaScript · lane B** — phases **9–12, 17, 18** (platform + applied) | session `b4ffc223` | 2026-08-14 | 🔴 **Active — LANE B of the two-way split.** DOM, events, network/storage, browser platform, machine coding, and the three kept storefront topics. **68 topics left**, all Understand/Know. Same rules as lane A: tier-locked to Understand+Know, Master is closed and not to be reopened, the scope cut applies, per-file save cadence. **Phase 9 Understand 07–13 ✅** — Traversal, Classes and styles, Forms, Removing and replacing, Batching, Layout thrashing, Measuring (all chunked; 22 files). Next: **phase 9 topic 14 · Scrolling**. See the [JavaScript claim notice](javascript/pages/README.md) for the lane table and the shared-file rules |
| **TypeScript · Part A** — phases **2–6** (the type system) | session `3bbe364c` | 2026-08-14 | 🔴 **Active — PART A of a two-way split (2026-08-14).** Narrowing, generics, classes, type-level programming, modules and the build — **73 topics**. Baseline: TypeScript is **30 of 187 topics**, phases 0–1 complete, phase 2 at 7/13. 🔴 **First task was a real defect** — `phase-2-narrowing/` had seven topic files and **no `README.md`**, the source of every TypeScript broken link in recent builds; the fix is to write the index and finish topics 08–13, **not** to change the link form. Part B (phases 7–12) is a separate session; **this lane never touches `phase-{7,8,9,10,11,12}-*/`**. Documentation-validated against the TypeScript handbook and release notes; phases 0–2 reuse the recorded `sandbox/ts-p{0,1,2}/` runs and carry **no fabricated console output**. Split rules: `devbible/project_typescript_split_parts_ab.md`; resume point: `devbible/progress_typescript_build.md`. |
...
| **Unclaimed**                              | —                  | —          | **Docker & Podman has zero pages** — the last unstarted technology, and a good place to start                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **[React](./react/README.md)**           | 4 parts · 210 topics | Pending — [229 pages](./react/pages/README.md), **phases 0–10 complete**; Phase 11 writing (10/17), Phase 14 writing (2/14); 12/13 dropped                                                                                                                                 |
