...
| **JavaScript · lane B** — phases **9–12, 17, 18** (platform + applied) | session `75e511e6` | 2026-08-14 | 🔴 **Active — LANE B of the two-way split**, and 🔴 **now written in the worktree `devbible-js-lane-b`, branch `js-lane-b`** (user's instruction, 2026-08-14) — **not merged into `main` yet**; the work is there and the merge is outstanding. DOM, events, network/storage, browser platform, machine coding, and the three kept storefront topics. **66 topics left**, all Understand/Know. Same rules as lane A: tier-locked to Understand+Know, Master is closed and not to be reopened, the scope cut applies, per-file save cadence. ✅ **Phase 9's Understand tier is COMPLETE — 07–15** (Traversal, Classes and styles, Forms, Removing and replacing, Batching, Layout thrashing, Measuring, Scrolling, Focus and accessibility; all chunked, 32 files). Next: **phase 9 topic 16 · `<dialog>`, the popover API and `inert`** (Know). See the [JavaScript claim notice](javascript/pages/README.md) for the lane table and the shared-file rules |
