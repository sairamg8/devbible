...
| **JavaScript** — all of `docs/javascript/` | session `c5329658` | 2026-08-14 | 🔴 **Active.** **Phases 3–6 Master tiers COMPLETE** — P3 01–08, P4 01/03–08, P5 01/02/04–07/09/10, P6 01–03. **Phase 7 Master tier COMPLETE** (all 11). **Phase 8 Master tier COMPLETE**. **Phase 9 Master tier COMPLETE**. **Phase 10 Master tier COMPLETE**. **Phase 11 Master tier COMPLETE** (all 5). **Phase 12 Master tier COMPLETE** (both). **Phase 13 Master tier COMPLETE** (all 3). **Phase 14 next**. See the [JavaScript claim notice](javascript/pages/README.md)                                                                                                                                             |
...
| **Express** — all of `docs/expressjs/`     | session `ffadd057` | 2026-08-14 | 🔴 **Active — Master-tier depth pass.** Structurally complete (11 phases, **114 of 114 topics**, 86 pages, all carrying `> Verified:`, a tier badge, Gotchas, Trade-off and Interview sections; every phase README has a Coverage table; 0 duplicate headings). **But depth does not follow tier**: all 28 Master topics sit between 63 and 200 lines and **not one is chunked**, against a corpus where PostgreSQL Master topics median 530 lines. The corpus was sized to the 300-line cap instead of to the topic. This pass rewrites all **28 Master topics to full depth**, chunking into `NN-topic/` directories past 300 lines. Additive — existing prose and console blocks are kept, never re-run, never invented.                                                                                                           |
...
| **Frontend toolchain** — 12 **new** folders: `docs/storybook/` `docs/vite/` `docs/webpack/` `docs/babel/` `docs/eslint-oxlint/` `docs/jest-rtl/` `docs/playwright/` `docs/redux-toolkit/` `docs/tanstack-query/` `docs/framer-motion/` `docs/web-vitals-performance/` `docs/frontend-architecture/` | session `0fe4e7e0` | 2026-08-14 | 🔴 **Active — importing a second corpus.** Bringing 12 tooling/testing technologies in from the separate `frontend-bible` repo (**202 files, ~25,400 lines, 0 stubs**) as new devbible folders. Work happens in worktree `devbible-frontend`, branch **`frontend-merge`**. **This lane touches no existing technology** — React, JavaScript, TypeScript, CSS and Git are owned by other live sessions and are explicitly out of scope here, as are Next.js and every backup directory in the source repo. Import is a **rewrite, not a copy**: source pages carry a different 4-section standard, a `[D]/[O]/[R]` badge system, and **zero `> Verified:` lines**. Pilot: **Storybook** — syllabus done (11 phases, 58 topics) and **Phases 0–2 complete (18 of 58 topics — all of Part 1)**; verified against `storybook@10.5.8`, with the Storybook 9 package consolidation written up because 12 of the 22 source files import packages that no longer exist. |
...
| **[JavaScript](./javascript/README.md)** | 5 parts · 337 topics | Pending — [182 pages](./javascript/pages/README.md); phases 0–2 complete, Master tiers of 3–13 complete, phase 14 under way                  |
...
| **[Express](./expressjs/README.md)**     | 4 parts · 114 topics | 🔴 **Master-tier depth pass** — [86 pages](./expressjs/pages/README.md) across 11 phases; all 114 topics covered and verified, but the 28 Master topics were sized to the cap (63–200 lines, none chunked) and are being rewritten to full depth. **6 of 28 done** — phases 0 and 1 complete and build-verified |
...
| Nginx                                    | Not started          | —                                                                                                                                          |
| **[Storybook](./storybook/README.md)**   | 4 parts · 58 topics  | Pending — [17 pages](./storybook/pages/README.md), **phases 0–2 complete** (18 topics, 4,390 lines — Part 1 of 4 done), phase 3 next. **First of 12 frontend-toolchain tracks being imported** — see the claim notice above |
