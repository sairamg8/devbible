...
| **JavaScript · lane B** — phases **9–12, 17, 18** (platform + applied) | session `75e511e6` | 2026-08-14 | 🔴 **Active — LANE B of the two-way split**, and 🔴 **now written in the worktree `devbible-js-lane-b`, branch `js-lane-b`** (user's instruction, 2026-08-14) — **not merged into `main` yet**; the work is there and the merge is outstanding. DOM, events, network/storage, browser platform, machine coding, and the three kept storefront topics. **60 topics left**. 🎉 ✅ **PHASE 9 · THE DOM IS COMPLETE — 19 of 19 at every tier, 59 files** (Master 01–06, Understand 07–15, Know 16–18, When Needed 19); this lane wrote 46 of those files, every topic chunked, every file under the 300-line cap. 🚧 **Now in phase 10 · Events and user input — topics 05–06 ✅** (Form and input events, Keyboard events), 07 · Pointer events next. Same rules as lane A: tier-locked to Understand+Know, Master is closed and not to be reopened, the scope cut applies, per-file save cadence. ⚠️ A full `yarn build` is currently **blocked by a React MDX error** inherited at the branch point (not lane B's file); verification meanwhile is a relative-link pass over `docs/javascript/pages` — **2,184 links, 0 broken, 0 files over 300**. See the [JavaScript claim notice](javascript/pages/README.md) for the lane table and the shared-file rules |
