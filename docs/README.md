...
| **Nginx** — all of `docs/nginx/`           | session `21fbf27e` | 2026-08-14 | 🔴 **Active — new track.** Work happens in worktree **`devbible-nginx`, branch `nginx`**, not the shared checkout, so a build break here is genuinely this track's fault. ✅ **Syllabus complete — 4 parts, 12 phases, 210 topics** (Master 58 / 28% · Understand 101 · Know 38 · When Needed 13), scoped to what a fullstack MERN/PERN developer needs in front of a Node app; the mail modules, media streaming, body-transform modules and NGINX Plus depth are deliberately out. 🔴 **The version fact the whole track turns on:** since **nginx 1.29.7** upstream `keepalive` is **on by default** (`keepalive 32 local;`), `proxy_http_version` defaults to **1.1**, and `Connection` is no longer sent — the keep-alive incantation in every tutorial is redundant on 1.30+ and still required on 1.28 and older. Target: **1.30.x stable** (1.30.4, 15 July 2026). Documentation-validated against nginx.org under the no-new-sandboxes rule; **there is no nginx sandbox in this repo and none is planned**. ✅ **Phase 0 COMPLETE — 14/14 topics, 10 pages** (the process model, sizing, signals, reload and live binary upgrade, `-t`/`-T`/`-V`, installing, modules, versions and Plus, the forks). Next unit: **Phase 1 · The configuration language (14 topics)**. |
...
| **[Nginx](./nginx/README.md)**           | 4 parts · 210 topics | 🔴 In progress — [10 pages](./nginx/pages/README.md), **phase 0 complete (14/14 topics)**; phase 1 next. Target nginx 1.30.x stable          |
