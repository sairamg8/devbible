...
| **JavaScript · chunk C** — phases **7, 8** (async + modules, errors, tooling) | session `f6dffd4a` | 2026-08-15 | 🔴 **Chunk C of a FOUR-way split (2026-08-15) — this replaces the old lane A/B split, which is closed.** 🚧 **22 topics left, 3 written** — ✅ **7/12 · Timers** (4 files, 822 lines), ✅ **7/13 · Creating promises** (3 files, 533 lines) and ✅ **7/14 · Cancellation** (3 files, 548 lines). Phase 7 (8 left: topics 15–22, timeouts/retries → backpressure) and phase 8 (14: topics 05–18, dynamic `import()` → linting). **Next: 7/15 · Timeouts, retries, backoff and jitter.** The natural pair: error handling, the event loop and the toolchain sit either side of the same seam. Tier-locked to **Understand and Know** — Master is closed at 99/99 and is not reopened for depth. The 2026-08-14 scope cut stands (21 dropped, 34 parked). Work on `main`, stage explicit paths, **never `git add -A`**. Resume file: `devbible/progress_javascript_split_4way.md`. See the [JavaScript claim notice](javascript/pages/README.md) |
