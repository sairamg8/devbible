...
**Symptom: an `async` component works, and the same pattern fails in a Client Component.** Cause: React states it directly — *"Since async components are not supported on the client, we await the promise with `use`."* The reason is the same "renders once" property: a Client Component re-renders, so an in-flight promise has no defined behaviour across renders. Fix: fetch on the server and pass data down, or pass the pending promise as a prop and read it with `use` on the client.
