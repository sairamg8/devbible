---
sidebar_position: 7
title: "The `proxy.ts` layer (successor to `middleware.ts`): request interception, rewrites, and its secu…"
sidebar_label: "The `proxy.ts` layer (successor to `middleware.ts`): request interception, rewrites, and its secu…"
description: "The `proxy.ts` layer (successor to `middleware.ts`): request interception, rewrites, and its security limits (never as the sole auth gate)."
---

# ▲ The `proxy.ts` layer (successor to `middleware.ts`): request interception, rewrites, and its secu…

> **Syllabus chapter:** 2. Routing and Navigation  
> **Exact concept:** The `proxy.ts` layer (successor to `middleware.ts`): request interception, rewrites, and its security limits (never as the sole auth gate).  
