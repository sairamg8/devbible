...
> Correction to the line above: the Caching page is **not** the only PPR reference in 16.3.4 — [Implementing Partial Prerendering on your platform](https://nextjs.org/docs/app/guides/ppr-platform-guide) (`version: 16.3.4`, `lastUpdated: 2026-03-30`) and the [glossary](https://nextjs.org/docs/app/glossary) (`lastUpdated: 2026-08-25`) also cover it; the body below now says so. The `<html>`-attribute quote is from [Migrating to Cache Components](https://nextjs.org/docs/app/guides/migrating-to-cache-components), re-checked verbatim.
...
⚠️ **A note on sourcing, because it affects what you can look up.** There is no *getting-started* page for Partial Prerendering in the 16.3.4 documentation — `/docs/app/getting-started/partial-prerendering` returns *"The URL `/docs/app/getting-started/partial-prerendering` does not exist."* The application-developer material lives inside the *Prerendering* section of the [Caching](https://nextjs.org/docs/app/getting-started/caching) page, plus a short [glossary](https://nextjs.org/docs/app/glossary) entry. Two other PPR pages do exist and are easy to miss: [Implementing Partial Prerendering on your platform](https://nextjs.org/docs/app/guides/ppr-platform-guide), written for platform engineers rather than app authors — it is where the build-time artefacts are named (*"a static HTML shell and a `postponedState` blob"*) — and the [Next.js 16 upgrade guide](https://nextjs.org/docs/app/guides/upgrading/version-16), which records the removal of the old flag. ⚠️ The platform guide carries `lastUpdated: 2026-03-30` against Caching's `2026-08-25`, so where the two describe behaviour differently, Caching is the newer document. If you go looking for a PPR guide and find only blog posts from the 15-era experimental flag, that is why.
...
The shell that ships to the CDN contains the header, the legend, the board columns, and *both skeletons*. The notification count is a hole.

⚠️ **One qualification on "the board columns", because it depends on a fact this route does not state.** `getColumns` is keyed by `team`, which comes from `params` — so the concrete columns can only be prerendered for the `team` values the build actually knows about:
...
With `generateStaticParams` listing the teams, the sentence above is literally true. Without it, the first visitor to a given team gets the App Shell — header, legend and *three* fallbacks — and ISR fills in the concrete version afterwards. That distinction is the whole subject of [03b](03b-maximizing-the-shell-the-app-shell-and-what-crawlers-get.md). Under the previous model that single `cookies()` read in `NotificationBell` would have made the entire route dynamic — header, legend, columns and all — and the page would have waited on the database before rendering a single byte.
...
**★ Symptom: you search the documentation for a Partial Prerendering page and find nothing, so you follow a 15-era tutorial about `experimental.ppr`.** Cause: `/docs/app/getting-started/partial-prerendering` 404s, and the app-author content lives inside the Caching page instead; the only page *titled* for PPR is the platform-engineering guide, which answers a different question. The flag that tutorial tells you to set was removed — [Migrating to Cache Components](https://nextjs.org/docs/app/guides/migrating-to-cache-components) puts it as *"Next.js 16 removes the experimental Partial Prerendering flag (`experimental.ppr`) and the `experimental_ppr` route segment config."* Fix: `cacheComponents: true` is the whole configuration; see [01](01-the-explicit-caching-model-cachecomponents-build-flag-and-th.md). A codemod exists for stripping `experimental_ppr` from segments.
...
Because PPR stopped being a feature. In 15 it was `experimental.ppr`, a flag with its own guide, something you opted into per route. In 16 it is what rendering does when `cacheComponents` is on — the flag and the `experimental_ppr` segment config were both removed rather than deprecated. So it is documented where it now belongs, inside the Caching page's *Prerendering* section, with a glossary entry. The one page still titled for PPR — *Implementing Partial Prerendering on your platform* — is addressed to people building a hosting platform, and is where the build artefacts are named: a static HTML shell plus an opaque `postponedState` blob that must be stored and revalidated atomically with it. That is useful if you are writing an adapter and irrelevant if you are writing pages. The practical consequence is that searching for a PPR guide surfaces mostly 15-era material describing an experimental flag that no longer exists, which is a real hazard when the tutorial's first instruction is to add configuration that will now do nothing.
