...
⚠️ **Criteria 3 and 4 assume the App Shell path, and the guide asks for a second flag.** The ISR-with-Cache-Components guide pairs them: *"Enable both `cacheComponents` and Partial Prefetching. Cache Components produces the App Shell, while Partial Prefetching upgrades it to a full route once the params are known."* `partialPrefetching` defaults to `false` and the ch4 scaffold does not set it. The documentation does not settle whether the background upgrade happens with `cacheComponents` alone, so treat that as unspecified: set `partialPrefetching: true` in `next.config.ts` before judging these two criteria, and note it requires `cacheComponents`, which you already have.
