...
> Validated: 2026-09-05 · claims + version spine re-checked against the Next.js 16.3.4 docs · session d2e9b9fe. Every quote, every DevTools label (**Page load**, **Client nav**, **Pause on navigations**, **Resume**) and the `instant()` signature re-checked verbatim against [Ensuring instant navigations](https://nextjs.org/docs/app/guides/instant-navigation) (`version: 16.3.4`, `lastUpdated: 2026-08-25`). Two sources the line above does not name but the page quotes: [Migrating to Cache Components](https://nextjs.org/docs/app/guides/migrating-to-cache-components) for the HTTP-response sentence, and [`instant`](https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config/instant) (`lastUpdated: 2026-08-03`) for the dev-prefetch caveat added below.
...
**★ Symptom: navigations feel sluggish in `next dev` even though every route validates green.** Cause: the dev server does not prefetch at all, so nothing the Inspector shows you about *timing* transfers to production — the segment-config reference states it directly: *"Next.js does not perform prefetches in development, so navigations may not feel as instant as they will in production. Validation reflects what will happen during `next start`, where prefetching is enabled."* Fix: use dev to judge **structure** — which content lands in the shell, where the fallbacks are — and judge *feel* against `next start`. This is also why an `instant()` suite run against `next dev` still means something: it asserts on the shell, not the clock. Running it against a production build needs `exposeTestingApiInProductionBuild`.
