...
Why it is static is the part almost everyone gets wrong, including earlier revisions of this page. It is **not** that the bare `fetch` went into the Data Cache. The `fetch` reference names the default `auto no cache` and spells out what actually happens:
...
Nothing on this route reads request-time data, so the route prerenders; the fetch therefore runs **once, during `next build`**, and the HTML that produced is what every visitor gets until the route revalidates. The staleness lives in the prerender, not in a data cache. To get fresh data you had to *say something* — `cache: 'no-store'`, `export const dynamic = 'force-dynamic'`, a `revalidate`, or any request-time API. Silence meant "build it once and serve that", which is the single most misread default in the framework's history; the corpus states the outcome at [ch4 · 03c](../04-data-fetching-in-the-app-router/03c-diagnosing-stale-and-unexpectedly-dynamic-routes.md): a bare `fetch()` leaves a route **static and stale**, not dynamic.

⚠️ **If you read "a bare `fetch()` is cached by default" anywhere — including in older revisions of this corpus — check it against 16.3.4 before repeating it.** The previous-model guide is flat about it: *"By default, `fetch` requests are not cached."* The observable outcome is identical, which is exactly why the wrong mechanism survives; the difference shows up the moment a request-time API appears above the call, because then the fetch runs per request and nothing was ever cached to fall back on. The boundary is Next.js 15, whose release post lists it as a breaking change: *"**Caching Semantics (Breaking):** `fetch` requests, `GET` Route Handlers, and client navigations are no longer cached by default."* (The body of that post elaborates only the `GET` handler and Client Router Cache halves, so treat the `fetch` half as sourced to the bullet.) Anything written for 13 or 14 is describing a `force-cache` default that no longer exists.
...
Under Cache Components it is an uncached read: the data is fetched on every request, and because it is uncached the framework requires you to handle it, either by wrapping the call in `use cache` or by putting the component behind a `<Suspense>` boundary. Under the previous model the same call left the route **static and stale** — but by a mechanism worth getting right: the response was not put in the Data Cache (the docs say *"Caching is opt-in"* and *"By default, `fetch` requests are not cached"*), the route was simply prerendered because nothing on it read request-time data, so the fetch ran once during `next build` and that build's HTML was served until the route revalidated. The behaviour inverted completely, and this is the single most common source of confusion when reading Next.js material written across the 15-to-16 boundary — a blog post is only correct relative to the model it was written for.
