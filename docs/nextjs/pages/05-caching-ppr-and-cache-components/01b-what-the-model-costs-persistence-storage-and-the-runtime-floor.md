...
The build id is *in the key*, and the `use cache` reference spells out the one thing that displaces it:
...
⚠️ Read that carefully before treating it as an escape hatch. It says `deploymentId` **replaces** the build ID in the key — it does not say a `deploymentId` you deliberately hold constant across releases is a supported way to share cache entries between deploys, and the documentation nowhere endorses that. `deploymentId` exists for [skew protection](https://nextjs.org/docs/app/api-reference/config/next-config-js/deploymentId), where the value is *supposed* to change per deploy. Treat "pin `deploymentId` to keep the cache warm" as unverified and untested here, not as a documented technique. A durable store on its own does not help either, because after a deploy you are asking it different questions. If you deploy ten times a day and your upstream is a rate-limited third-party API, that is ten cold starts a day against it, and the fix is not a cache setting — it is an architectural one, such as putting your own cache in front of the upstream, below Next.js entirely.
...
**★ Symptom: cached data recomputes after every deploy, even though you configured a durable remote cache handler.** Cause: the cache key includes the build id, so a new deployment asks the store for keys it has never seen. Nothing is wrong with the handler. Fix: the documentation gives one supported answer and it is not a cache setting — *"For data that needs to persist across deploys, use `unstable_cache` for non-`fetch` functions or the `fetch` cache"*, i.e. keep that particular read on the previous model. The other answer is to put a cache you own in front of the upstream, outside the framework, so that Next's cold start hits your store rather than the third party.
...
It means every deployment is a distinct cache namespace, so no `use cache` entry from the previous build is reachable after a deploy — including entries in a durable remote handler. The practical effect is a guaranteed cold cache at every release, sized by however much traffic you serve. There is no cache setting that turns it off. `deploymentId`, if configured, *"overrides the build ID for cache key purposes"* — but that key is meant to change per deploy for skew protection, and the docs do not present freezing it as a caching technique, so do not present it as one in an interview either. The responses that *are* documented or architectural: leave the deploy-spanning reads on `unstable_cache` or the `fetch` cache, which the `use cache` reference explicitly recommends for data that must persist across deploys; reduce deploy frequency; accept the cold start and size the upstream for it; or place a cache you control between the application and the upstream so the framework's cold start hits your store rather than a rate-limited third party. The last option is the only one that removes the problem entirely, and it lives outside the framework.
