...
Next.js supports `app/` and `pages/` **coexisting in the same project** specifically to make this an incremental migration rather than a big-bang rewrite — for any path defined in both directories, **`app/` wins** (see [file conventions](../02-routing-and-navigation/01-file-system-routing-pagetsx.md) and the [legacy Pages Router reference](../17-advanced-ecosystem-topics/02-pages-router-app-router-migration-roadmaps-for-legacy-codeba.md) for the precedence rule and coexistence caveats this recipe builds on). This means you can migrate one route, ship it, verify it, and move to the next — the rest of the app keeps working unmodified on `pages/` throughout.
