...
🔴 **This chapter's canonical schema does NOT carry this index, and that is a deliberate ruling rather than an omission.** [02 · The schema and the migration story](02-the-schema-and-the-migration-story.md) states it: `position` is not `UNIQUE`, because making it unique *converts a cosmetic degradation into a `23505` on a drag-and-drop* — and [07g · Position collisions and `updatedAt`](07g-position-collisions-and-updatedat.md) reaches the same conclusion from the update side, where transient ties during a reorder are normal and legal. The remedy below is presented in full because it is genuinely correct for a board that has no drag-and-drop, and because knowing what it forbids is how you understand why the schema declines it. **On the SprintDesk schema, remedy 2 or 3 is the one to reach for.**
...
**★ Symptom: two cards created within a second of each other share a position, and the board's order changes between refreshes.** Cause: `max(position) + 1` read in one statement and written in another; under READ COMMITTED both requests read the same snapshot and computed the same value, and no constraint objected. Fix: pick one of the three remedies above. The unique partial index plus a bounded retry is the smallest change and is right for an append-only board; **on SprintDesk, where cards are dragged, the advisory lock is the one this chapter takes** — see the ruling under remedy 1.
