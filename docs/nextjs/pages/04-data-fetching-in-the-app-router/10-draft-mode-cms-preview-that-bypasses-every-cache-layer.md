...
> *"`fetch()` calls skip the Next.js fetch cache and hit the network directly. · Components and functions inside `'use cache'` re-execute on every request, and their results are not saved to the cache. · `unstable_cache` reads and writes are bypassed in the same way. · The page is excluded from the ISR response cache and is served with `Cache-Control: private, no-cache, no-store, max-age=0, must-revalidate`."*
...
> *"Your data-fetching code does not need to change if your CMS serves draft and published content from the same URL."*

That is the design goal. The page does not know it is previewing. It fetches as it always does, and the framework declines to answer from any cache.

## The contract with the CMS

The guide states its assumptions explicitly:

> *"Your headless CMS supports configurable preview URLs (most do). · The CMS opens a URL like `/api/draft?secret=XXX&slug=/posts/foo` in a new tab when an editor clicks "Preview". The secret is a shared token; the slug is the path to preview. · Your Next.js app validates the secret, enables Draft Mode, and redirects to the slug."*

And it is unusually candid about the HTTP-method compromise it is making:

> *"`GET` is meant to be a safe, read-only method. Operations that affect future requests, like enabling Draft Mode via a cookie, should use `POST`. The entry handler uses `GET` because we're assuming a CMS preview integration: the CMS opens the URL in a new browser tab, which is a `GET` request. The exit flow in Step 4 uses `POST`."*
...
> *"As written, the handler is public: anyone who hits `/api/draft` enables Draft Mode for themselves."*
...
**The redirect target comes from `post.slug`, not from `searchParams`.** The comment in the docs is explicit: *"Redirect to the path from the fetched post, not from searchParams, to avoid open redirect vulnerabilities."* `redirect(slug)` with an attacker-supplied `slug` is an open redirect on an endpoint whose URL, including the secret, editors paste into chat.
...
The `draftMode` reference says the same from the other side: *"If calling the route using the `<Link>` component, you must pass `prefetch={false}` to prevent accidentally deleting the cookie on prefetch."*
...
`isEnabled` is the one runtime read permitted inside a caching directive:

> *"`isEnabled` is readable inside a caching directive scope. Other runtime APIs like `cookies()` and `headers()` are not allowed inside caching directive scopes, even when Draft Mode is active."*
...
But the toggles are not permitted there:
...
| Fact | Source |
| --- | --- |
| Cookie name is `__prerender_bypass` | `enable()` reference |
| A new bypass cookie value is generated on every `next build` | *"This ensures that the bypass cookie can't be guessed."* |
| Session ends when the browser closes, by default | *"By default, the Draft Mode session ends when the browser is closed."* |
| Local HTTP testing needs third-party cookies and local storage allowed | *"To test Draft Mode locally over HTTP, your browser will need to allow third-party cookies and local storage access."* |
...
`Link` prefetches by default, so the browser fetches the disable endpoint while the editor is still reading the page. Draft Mode turns off before any click and the preview appears to randomly stop working. Use a form (*"Forms are not prefetched, regardless of method"*) or, if you must use a link, `prefetch={false}`.
...
*"A new bypass cookie value will be generated each time you run `next build`."* Editors mid-review lose Draft Mode on every deploy and have to re-enter through the CMS. That is correct behaviour — the cookie must be unguessable — but it will be reported as a bug unless the team knows.
...
Draft Mode does not relax the cache directive's rules. *"Other runtime APIs like `cookies()` and `headers()` are not allowed inside caching directive scopes, even when Draft Mode is active."* Only `isEnabled` is special-cased.
...
Because `Link` prefetches by default, and a prefetch of the disable endpoint clears the Draft Mode cookie before the editor clicks anything. The docs are explicit: *"Forms are not prefetched, regardless of method."* If a link is unavoidable, it needs `prefetch={false}`.
...
Because Draft Mode suspends the caching, not just the reads: *"all functions and components under a caching directive scope re-execute on every request and results are not saved to the cache."* The `'use cache'` directive is effectively inert for that request, which is why the same component code serves both audiences correctly.
...
Only where you read from. Branch the base URL on `isEnabled` inside your data function; everything else stays the same, because *"The cache bypass still applies to both branches; the fork only chooses where to read from."* The page component still does not need to know it is previewing.
