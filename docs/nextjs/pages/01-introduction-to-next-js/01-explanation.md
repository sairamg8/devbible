...
**★ Symptom: a tutorial's code does not compile, and the API it uses does not exist.** Cause: App Router material written across four majors is all still online, and the framework changed substantially in each. `request.ip` and `request.geo` were removed in Next.js 15; `GET` Route Handlers stopped being cached by default in Next.js 15; and `v16.0.0` — *"`dynamic`, `dynamicParams`, `revalidate`, and `fetchCache` removed when Cache Components is enabled"*. Fix: check the publication date, and prefer the docs bundled in your own `node_modules/next/dist/docs/` — they match your installed version by construction.

**★ Symptom: an AI agent writes confident App Router code against APIs that were removed.** Cause: it is answering from training data. Fix: two separate mechanisms, and it is worth keeping them straight. `next upgrade` refreshes the docs bundled inside the package — *"Upgrading also updates the documentation bundled inside the `next` package at `node_modules/next/dist/docs/`"* — while `next dev` is what writes the pointer to them: *"Running `next dev` writes and maintains a version-matched `AGENTS.md` block that points directly to the bundled docs in your project's local node modules."* Vercel retired its earlier Skills on the strength of it — *"we're retiring our earlier Skills that existed solely to bring current documentation to your apps"* — so this is now the supported path, and it only works if you actually upgrade.
...
Because the App Router changed substantially across four majors and all of it is still indexed. Concretely: `request.ip` and `request.geo` were removed in Next.js 15 *"as these values are provided by your hosting provider"*, and a property that no longer exists reads `undefined`, so a rate limiter keyed on it silently collapses to one bucket; `GET` Route Handlers stopped being cached by default in Next.js 15, which inverts a widely-repeated pitfall; and the route segment config reference records against `v16.0.0` that *"`dynamic`, `dynamicParams`, `revalidate`, and `fetchCache` removed when Cache Components is enabled"*. The defence is to prefer the docs bundled at `node_modules/next/dist/docs/`, which match your installed version by construction rather than by the author's memory.
