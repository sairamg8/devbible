...
| 08 | **[Utility types in app code](08-utility-types-in-app-code/README.md)** *(9 chunks)* | <span className="db-tier t-know">Know</span> | Derive, never redeclare — and 🔴 `Omit` silently accepts keys that do not exist, `satisfies` versus annotation versus `as`, branded `ProductId` vs `OrderId` |
