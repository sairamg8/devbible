...
| **The build: `@angular/build`** — 🔴 **three tools, not two, since 22.1.0**: esbuild compiles and bundles, **Rolldown** re-bundles the chunks (`useRolldownChunks … ?? true`, escape hatch `NG_BUILD_CHUNKS_ROLLDOWN`), Vite serves the dev server; the Webpack builders are legacy and only kept for old configurations | <span className="db-tier t-understand">Understand</span> |
...
| **The release train** — 🔴 **a major every 12 months since v22** (angular.dev: *"Until Angular v22, Angular had a 6-month major release cycle"*), 4–6 minors each, and *"All major releases are typically supported for 24 months"* — plus how to read a changelog for the breaking changes that matter to you | <span className="db-tier t-understand">Understand</span> |
...
| JIT vs AOT — where JIT still exists (`TestBed`, and `platform-browser-dynamic` — ⚠️ **npm-deprecated at 22.1.5**: *"@angular/platform-browser-dynamic is deprecated. Use `@angular/platform-browser` instead."*), and why it is not a deployment option | <span className="db-tier t-know">Know</span> |
