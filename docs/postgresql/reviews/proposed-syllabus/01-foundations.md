...
counterpart per the [example policy](./README.md#example-policy--the-hard-rule-for-this-syllabus)
...
| **Foreign keys** — `REFERENCES`, and every `ON DELETE`/`ON UPDATE` action spelled out | <span className="db-tier t-master">Master</span> |
