...
An unrestored backup is not a backup. After every meaningful dump strategy change, restore into a scratch database and run a smoke query.


`psql` is how you prove every later claim. pg_dump and pg_restore is daily operator skill.
...
**★ What is the core idea of “pg_dump and pg_restore”?**  
Logical backups with format choice; restore a single table when needed.
...
← [App role should not own schema](03-app-role-not-owner.md) · Next → [pg_hba.conf](05-pg-hba.md)
