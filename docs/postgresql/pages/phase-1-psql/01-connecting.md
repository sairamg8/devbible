...
sidebar_label: "01 · Connecting with psql"
sidebar_position: 1
---

<span className="db-tier t-master">Master</span>

**-h/-p/-U/-d, URIs, into a container.**

## Why it matters

`psql` is how you prove every later claim. Connecting with psql is daily operator skill.
...
const {rows} = await pool.query('select $1::text as topic', ['Connecting with psql']);
console.log(rows[0]);
await pool.end();
```
...
**★ What is the core idea of “Connecting with psql”?**  
-h/-p/-U/-d, URIs, into a container.
...
← [Phase 1 index](README.md) · Next → [Daily meta-commands](02-daily-meta-commands.md)
