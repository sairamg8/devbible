...
**Client vs server file access.**

## Why it matters

`psql` is how you prove every later claim. copy vs server COPY is daily operator skill.
...
const {rows} = await pool.query('select $1::text as topic', ['copy vs server COPY']);
console.log(rows[0]);
await pool.end();
```
...
**★ What is the core idea of “copy vs server COPY”?**  
Client vs server file access.
