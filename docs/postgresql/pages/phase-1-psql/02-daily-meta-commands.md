...
**\l \c \dt \dn \df \di \dv \ds.**

## Why it matters

`psql` is how you prove every later claim. Daily meta-commands is daily operator skill.
...
const {rows} = await pool.query('select $1::text as topic', ['Daily meta-commands']);
console.log(rows[0]);
await pool.end();
```
...
**★ What is the core idea of “Daily meta-commands”?**  
\l \c \dt \dn \df \di \dv \ds.
