...
**SQL files and seed scripts.**

## Why it matters

`psql` is how you prove every later claim. Include files (i / ir) is daily operator skill.
...
const {rows} = await pool.query('select $1::text as topic', ['Include files (i / ir)']);
console.log(rows[0]);
await pool.end();
```
...
**★ What is the core idea of “Include files (i / ir)”?**  
SQL files and seed scripts.
