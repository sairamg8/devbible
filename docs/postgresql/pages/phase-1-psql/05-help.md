...
`psql` is how you prove every later claim. Help (? and h) is daily operator skill.
...
const {rows} = await pool.query('select $1::text as topic', ['Help (? and h)']);
console.log(rows[0]);
await pool.end();
```
...
**★ What is the core idea of “Help (? and h)”?**  
Built-in reference.
