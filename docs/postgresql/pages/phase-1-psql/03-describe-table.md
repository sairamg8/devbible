---
title: "Describe table (d / d+) in full"
sidebar_label: "03 · d and d+ in full"
sidebar_position: 3
---

<span className="db-tier t-master">Master</span>

**Columns, indexes, constraints, FKs.**

## Why it matters

`psql` is how you prove every later claim. Describe table (d / d+) in full is daily operator skill.
...
const {rows} = await pool.query('select $1::text as topic', ['Describe table (d / d+) in full']);
console.log(rows[0]);
await pool.end();
```
...
```console
$ node example.mjs
{ topic: 'Describe table (d / d+) in full' }
```
...
**★ What is the core idea of “Describe table (d / d+) in full”?**  
Columns, indexes, constraints, FKs.
