...
**\x, \pset, csv, unaligned.**

## Why it matters

`psql` is how you prove every later claim. Output control is daily operator skill.
...
const {rows} = await pool.query('select $1::text as topic', ['Output control']);
console.log(rows[0]);
await pool.end();
```
...
**★ What is the core idea of “Output control”?**  
\x, \pset, csv, unaligned.
