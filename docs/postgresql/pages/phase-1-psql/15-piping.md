...
`psql` is how you prove every later claim. Piping psql is daily operator skill.
...
const {rows} = await pool.query('select $1::text as topic', ['Piping psql']);
console.log(rows[0]);
await pool.end();
```
...
**★ What is the core idea of “Piping psql”?**  
Reports and pipelines.
...
← [\errverbose and SQLSTATE](14-errverbose.md) · [Phase 1 index](README.md)
