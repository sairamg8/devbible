...
**NULL means unknown; = NULL is never true; use IS NULL / IS DISTINCT FROM.**

## Why it matters

Type choices are expensive to reverse. Getting NULL semantics wrong creates classes of bugs (money, time zones, ids) that appear only under load or in another region.
...
**★ What is the core idea of “NULL semantics”?**  
NULL means unknown; = NULL is never true; use IS NULL / IS DISTINCT FROM.
