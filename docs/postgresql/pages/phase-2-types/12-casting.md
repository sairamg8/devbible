...
**Explicit casts are fine; wrapping a column in a cast in WHERE often prevents index use.**

## Why it matters

Type choices are expensive to reverse. Getting Casting wrong creates classes of bugs (money, time zones, ids) that appear only under load or in another region.
...
**★ What is the core idea of “Casting”?**  
Explicit casts are fine; wrapping a column in a cast in WHERE often prevents index use.
