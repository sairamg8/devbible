...
**timestamptz stores UTC instants; session TimeZone affects display, not the absolute meaning.**

## Why it matters

Type choices are expensive to reverse. Getting Time zones wrong creates classes of bugs (money, time zones, ids) that appear only under load or in another region.
...
**★ What is the core idea of “Time zones”?**  
timestamptz stores UTC instants; session TimeZone affects display, not the absolute meaning.
