...
**Use numeric for money and exact decimals; float only for scientific approximations.**

## Why it matters

Type choices are expensive to reverse. Getting numeric vs float wrong creates classes of bugs (money, time zones, ids) that appear only under load or in another region.
...
**★ What is the core idea of “numeric vs float”?**  
Use numeric for money and exact decimals; float only for scientific approximations.
