...
**Store event time as timestamptz; avoid timestamp without time zone for instants.**

## Why it matters

Type choices are expensive to reverse. Getting timestamptz vs timestamp wrong creates classes of bugs (money, time zones, ids) that appear only under load or in another region.
...
**★ What is the core idea of “timestamptz vs timestamp”?**  
Store event time as timestamptz; avoid timestamp without time zone for instants.
