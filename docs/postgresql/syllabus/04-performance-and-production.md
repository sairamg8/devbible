...
The largest part, and the one most defensible to defer writing until Wave B
(see parked notes in `reviews/syllabus-re-review.md`). Master is deliberately
thin here: this is material you must be able to *reason about*, not type from
memory.
...
📖 **Explanation written:** [Phase 10 — Indexes](../pages/phase-10-indexes/)


*18 topics.* Taught after the SQL phases on purpose — an index only makes sense
...
| **Indexing foreign key columns** — PostgreSQL does not auto-index the referencing side; the DELETE that goes sequential because of it | <span className="db-tier t-understand">Understand</span> |
...
| **Index-only scans**, `INCLUDE` covering columns, and the visibility map | <span className="db-tier t-understand">Understand</span> |
...
📖 **Explanation written:** [Phase 11 — MVCC](../pages/phase-11-mvcc/)
...
📖 **Explanation written:** [Phase 13 — Ops](../pages/phase-13-ops/)


*18 topics.* The gap between "works on my machine" and "runs for three years".
Backups are Master (`pg_dump`/`pg_restore` plus a practiced restore); the scheduled disaster drill stays Know.
...
| **`pg_dump` and `pg_restore`** — logical backups, format choice, restoring a single table, and **restore once to verify** | <span className="db-tier t-master">Master</span> |
